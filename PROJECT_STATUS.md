...
- 支付回调事件支持按筛选条件导出 CSV，便于对账和故障工单排查。
- 运营概览和系统状态会预警滞留回调、处理失败及金额不匹配事件。
...
- `/api/health` 健康检查接口。
- 已于 2026-06-15 生成并安全审核生产数据库 `public` schema 纯结构快照及 SHA256 校验文件。
- GitHub 代码质量工作流会自动验证所有数据库结构快照的校验值、核心结构和敏感信息安全。
...
- Resend 发件子域名 `mail.jizhidao-ai.com` 已完成 DNS 验证；生产环境已验证管理员新订单提醒和用户到账邮件均可正常送达。
...
5. `20260615_payment_webhook_safety.sql`
   - 增加支付平台订单号和交易流水号
   - 增加支付回调事件幂等账本
   - 增加在线支付金额校验和原子加点函数
   - 已于 2026-06-15 在生产 Supabase 数据库执行
...
1. 充值订单迁移文件已确认使用 UTF-8 中文文本；下次生产充值时继续抽查点数流水描述。

以下生产流程已经验收通过：

- 普通邮箱 Magic Link 注册与登录。
- `/admin/orders` 加载、管理员确认到账和用户点数增加。
- 管理员新订单提醒邮件和普通用户到账邮件。
- 2026-06-14 域名 DNS 故障已恢复，详见 `docs/INCIDENT_LOG.md`。

## 8. 后续开发顺序

1. 确定首个支付平台和商户产品，执行支付安全迁移后接入创建支付与签名回调适配器。
2. 继续完成移动端、性能、备份和异常恢复测试。
